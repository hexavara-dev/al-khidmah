...
import { Home, ScrollText, Banknote, CircleUser, HandCoins, HardHat } from 'lucide-react';
...
const navItems: NavItem[] = [
    { key: 'ppob',    label: 'PPOB',    icon: Home,       href: '/',        matchPaths: ['/'] },
...
export default function BottomNav({ active }: Props) {
    ...
    return (
        <>
            <div className="fixed bottom-6 left-0 right-0 z-50 flex justify-center pointer-events-none">
                <div className="flex justify-center pointer-events-auto">
                    <nav className="inline-flex items-center gap-0.5 rounded-3xl border border-outline-variant/20 bg-surface-container-lowest/90 px-2 py-3 shadow-xl shadow-on-surface/10 backdrop-blur-xl">
                        {navItems.map(({ key, label, icon: Icon, href, wip }) => {
            ...
            {/* WIP toast */}
            <div className={`fixed bottom-28 left-1/2 z-60 -translate-x-1/2 transition-all duration-300 ${
                wipToast ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0 pointer-events-none'
            }`}>
                <div className="flex items-center gap-2.5 rounded-2xl border border-outline-variant/20 bg-on-surface px-4 py-3 shadow-xl">
                    ...
                    <p className="text-xs font-semibold text-surface-bright">Fitur ini sedang dalam pengembangan</p>
                </div>
            </div>
        </>
    );
